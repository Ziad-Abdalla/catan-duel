...
function gen(s: GameState, rng: Rng): Action {
  ...
  const factories: Array<() => Action> = [
    ...
    () => ({ type: 'movePlaced', player: p, placedIndex: ri(Math.max(1, placed.length)), slot: pick(SLOTS) ?? 's0-up' }),
    () => ({ type: 'shuffleStack', stackIndex: ri(Math.max(1, s.drawStacks.length)) }),
    () => ({ type: 'takeFromStack', player: p, stackIndex: ri(Math.max(1, s.drawStacks.length)), position: rng() < 0.5 ? ri(12) : undefined }),
    () => (hand.length ? { type: 'putToStack', player: p, cardId: pick(hand), stackIndex: ri(Math.max(1, s.drawStacks.length)), position: rng() < 0.5 ? 'top' : 'bottom' } : { type: 'resolveBrigand' }),
    () => ({ type: 'resolveBrigand' }),
